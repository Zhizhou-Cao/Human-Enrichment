...
export const MapCanvas: React.FC<MapCanvasProps> = ({ 
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (draggedNode) {
      ...
      if (rect) {
        ...
        // Explicitly cast to CategoryID to fix potential unknown type issues
        onUpdateCategoryPos(draggedNode as CategoryID, x, y);
      }
    } else if (isPanning) {
      setOffset(prev => ({
  ...
  const handleMouseUp = (e: React.MouseEvent, id?: string, isSub?: boolean, catId?: string) => {
    ...
    if (dragDuration < 200) {
      if (isSub && catId) {
        // Explicitly cast to CategoryID to fix potential unknown type issues
        onSelectNode(catId as CategoryID, id);
      } else if (id && !isSub) {
        onSelectNode(id as CategoryID);
      }
    }
    setIsPanning(false);
    setDraggedNode(null);
  ...
  const isNodeActive = (catId: string, subName?: string) => {
    return completedTasks.some((record: UserTaskRecord) => {
  ...
  const subNodes = useMemo(() => {
    ...
    categories.forEach((cat: CategoryInfo) => {
      ...
      catSubCats.forEach((scName: string) => {
  ...
  const spotlightPos = useMemo(() => {
    // Explicitly handle highlightedNodeId to avoid unknown type issues when used as key
    const targetId = highlightedNodeId;
    if (!targetId) return null;
    
    const catPos = categoryPositions[targetId as CategoryID];
    if (catPos) return catPos;
    
    const sub = subNodes.find(n => n.name === targetId);
    ...
    >
      ...
      >
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          {completedTasks.map((record: UserTaskRecord, i: number) => {
            const task = allTasks.find(t => t.id === record.taskId);
            if (!task) return null;
        ...
        {categories.map((cat: CategoryInfo, i: number) => {
          const pos = categoryPositions[cat.id] || { x: cat.x, y: cat.y };
          return (
            <div 
              key={cat.id} 
